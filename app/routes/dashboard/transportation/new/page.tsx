...
export default function NewTransportationServicePage() {
  ...
  useEffect(() => {
    if (!isLoading && !user) {
      router.push('/routes/auth/login?returnUrl=/routes/dashboard/transportation/new');
    } else if (user) {
      fetchProfile();
    }
      ...
      <div className="container mx-auto py-12 px-4">
        ...
        <div className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 p-6 rounded-md flex items-start max-w-2xl mx-auto">
          ...
          <div>
            ...
            <p className="mb-4">
              Only users with an Owner account can create transportation services. Your current account type is: <span className="font-semibold capitalize">{profile?.user_type || 'User'}</span>
            </p>
            <p className="text-sm">
